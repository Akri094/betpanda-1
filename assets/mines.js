...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-7d2e2826";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-7d2e2826">'+'<span class="tile-face-7d2e2826 tile-front-7d2e2826"></span>'+'<span class="tile-face-7d2e2826 tile-back-7d2e2826"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-7d2e2826")){return;}
var back=tile.querySelector(".tile-back-7d2e2826");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-7d2e2826");if(isMine){tile.classList.add("boom-7d2e2826");}
if(dim){tile.classList.add("dimmed-7d2e2826");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-7d2e2826")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-7d2e2826");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-7d2e2826")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-7d2e2826");void panelEl.offsetWidth;panelEl.classList.add("jolt-7d2e2826");}
endRound();}
